...
export default function AreasPage() {
  ...
  const items = useItemsByType('area');
  ...
  const handleCreate = async () => {
    try {
      const id = await createQuick('area');
      notifier.success('Área criada');
      navigate(`/item/${id}`);
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteItem(id);
      notifier.success('Área excluída');
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
  ...
  return (
    <Stack spacing={2}>
      <ListToolbar title="Áreas" search={search} onSearchChange={setSearch} onCreate={handleCreate} />
      {isSeeding && <LoadingState message="Carregando dados..." />}
      {filteredItems.length === 0 ? (
        <EmptyState
          icon={<GridViewOutlined />}
          title="Nenhuma area"
          description="Crie uma area para agrupar seus projetos."
